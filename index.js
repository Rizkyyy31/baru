...
kurr.sendMessage(id, buttonMessages, MessageType.buttonsMessage, options)
   ...
   // owner
    ...
    const htod = "6285975203515@s.whatsapp.net"
	...
	try {
		...
		if (anu.action == 'add') {
			...
			try {
				...
				sendButImage(anu.jid, teks, `Araa Chan`, img,but = [{buttonId:`donasi`, 
			...
			try {
				...
				sendButImage(anu.jid, teks, `Araa Chan`, img,but = [{buttonId: `Hello World!`, buttonText: {displayText: `SAYONARA !!`}, type: 1}], options = {contextInfo: {mentionedJid: [num, htod]}})
...
kurr.on('group-update', async (anu) => {
    ...
    kurr.on('chat-update', async (message) => {
        ...
        ownerNumber = ["6285975203515@s.whatsapp.net","6285975203515@s.whatsapp.net",`${settings.NomorOwner}@s.whatsapp.net`]
        dtod = "6285975203515@s.whatsapp.net"
